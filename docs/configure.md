...
An optional alternative name for this cluster, letting you run multiple queues on a single broker or Django site (cluster name and queue name are interchangeable).
You can also override it at startup with the `Q_CLUSTER_NAME` environment variable, or with `qcluster --name <name>`; this is also how a matching [alt-clusters](#alt-clusters) entry is selected.
Defaults to the value of [name](#name).

## log_level

The logging level for the `django-q` logger. Accepts any standard Python logging level name, e.g. `'DEBUG'`, `'INFO'` or `'WARNING'`. Defaults to `'INFO'`.

## workers

The number of workers to use in the cluster. Defaults to CPU count of the current host, but can be set to a custom number.  [^f1]
...
Only supported on platforms that implement the python resource module or install the [psutil](install.md#psutil_package) module.
Defaults to `None`.
...
See [retry](#retry) for details how to set values for timeout and retry.
...
When set to `True`, also acknowledge unsuccessful tasks. This causes failed tasks to be considered as successful deliveries, thereby removing them from the task queue. Can also be set per-task by passing the `ack_failure` option to `async_task`. Defaults to `False`.
...
The value must be bigger than the time it takes to complete longest task, i.e. [timeout](#timeout) must be less than retry value and all tasks must complete
...
When set to `True` this configuration option forces all `async_task` calls to be run with `sync=True`.
...
For more information on these settings please refer to the [Redis-py](https://github.com/redis/redis-py) documentation

<a id="django_redis"></a>
## django_redis

If you are already using [django-redis](https://github.com/niwinz/django-redis) for your caching, you can take advantage of its excellent connection backend by supplying the name
...
All connection keywords are supported. See the [iron-mq](https://github.com/iron-io/iron_mq_python#configure) library for more info

<a id="sqs_configuration"></a>
## sqs
...
The `mongo` dictionary can contain any of the parameters exposed by pymongo's [MongoClient](https://pymongo.readthedocs.io/en/stable/api/pymongo/mongo_client.html#pymongo.mongo_client.MongoClient)
...
For some brokers, you will need to set up the Django [cache framework](https://docs.djangoproject.com/en/stable/topics/cache/#setting-up-the-cache)
...
Used internally to manage workarounds during Django Q2's own test suite. Leave it at its default for normal operation. Defaults to `False`.

<a id="error_reporter"></a>
## error_reporter

You can redirect worker exceptions directly to various error reporters (for example [Rollbar](https://rollbar.com/) or [Sentry](https://docs.sentry.io/)) by installing Django Q2 with the necessary [extras](https://setuptools.readthedocs.io/en/latest/setuptools.html#declaring-extras-optional-features-with-their-own-dependencies).
...
For more information on error reporters and developing error reporting plugins for Django Q2, see [errors](errors.md).
...
!!! note
    The `cpu_affinity` setting requires the optional [psutil](install.md#psutil_package) module.
...
See [multiple queues](cluster.md#multiple-queues).
...
[^f1]: Uses `multiprocessing.cpu_count()` which can fail on some platforms. If so , please set the worker count in the configuration manually or install [psutil](install.md#psutil_package) to provide an alternative cpu count method.

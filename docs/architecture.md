...
![Django Q2 schema](assets/cluster.png)

## Signed Tasks

Tasks are first pickled and then signed using Django's own `django.core.signing` module using the `SECRET_KEY` and cluster name as salt, before being sent to a message broker. This ensures that task
...
!!! note
    When the [ack_failures](configure.md#ack_failures) option is set to `False` (the default), a task is
...
A worker process pulls a task of the Task Queue and it sets a shared countdown timer with [sentinel](#sentinel) indicating it is about to start work.
...
pusher and instruct the workers and monitor to finish the remaining items. See [stop_procedure](#stop_procedure)
...
!!! warning
    ...
    You can manage the amount of tasks in a clusters memory by setting the [queue_limit](configure.md#queue_limit).

...
!!! tip
    The maximum number of successful tasks can be set using the [save_limit](configure.md#save_limit) option.
...
If you want a schedule to only run a finite amount of times, e.g. every hour for the next 24 hours, you can do that using the `Schedule.repeats` attribute.
In this case you would set the schedule type to `Schedule.HOURLY` and the repeats to `24`. Every time the schedule runs the repeats count down until it hits zero and schedule is no longer run.
...
An exception to this are schedules of type `Schedule.ONCE`. Negative repeats for this schedule type will cause it to be deleted from the database.
...
!!! note
    if you have set the [save_limit](configure.md#save_limit) configuration option to not save successful tasks to the database, you will only see the failed results of your schedules.

Uses the `Schedule` model

## Queued tasks

This admin view is only enabled when you use the [ORM broker](brokers.md#orm_broker) broker.
